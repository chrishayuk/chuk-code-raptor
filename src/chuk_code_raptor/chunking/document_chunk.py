...
class SemanticDocumentChunk(SemanticChunk):
    ...
    def _analyze_structure(self):
        ...
        # Detect section type if not set
        if self.section_type == "paragraph":
            if content_clean.startswith('#'):
                self.section_type = "heading"
                self.heading_level = len(content_clean) - len(content_clean.lstrip('#'))
            elif content_clean.startswith('```') or content_clean.startswith('    '):
                self.section_type = "code_block"
            elif content_clean.startswith('-') or content_clean.startswith('*') or content_clean.startswith('1.'):
                self.section_type = "list"
            elif '|' in content_clean and content_clean.count('|') > 2:
...
def calculate_document_quality_metrics(chunk: SemanticDocumentChunk) -> Dict[str, float]:
    ...
    if chunk.sentence_count > 0 and chunk.avg_sentence_length > 5:
        metrics['completeness'] = min(chunk.sentence_count / 5.0, 1.0)
    else:
